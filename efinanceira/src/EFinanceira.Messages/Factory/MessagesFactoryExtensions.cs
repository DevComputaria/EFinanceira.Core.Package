...
public static class MessagesFactoryExtensions
{
    ...
    public static EFinanceiraMessageFactory AddEventos(this EFinanceiraMessageFactory factory)
    {
        ...
        // Registro do evento de Movimentação de Operação Financeira
        factory.RegisterFactory(
            MessageKind.Evento("EvtMovimentacaoFinanceira"),
            "v1_2_1",
            (Action<object>? seed) =>
            {
                var builder = new EvtMovimentacaoFinanceiraBuilder("v1_2_1");
        ...
        // Registro do evento de Cadastro de Patrocinado
        factory.RegisterFactory(
        ...
        // Registro do evento de Exclusão
        factory.RegisterFactory(
            MessageKind.Evento("EvtExclusao"),
            "v1_2_0",
            (Action<object>? seed) =>
            {
                var builder = new EvtExclusaoBuilder("v1_2_0");
        ...
        // Registro do evento de Fechamento e-Financeira
        factory.RegisterFactory(
            MessageKind.Evento("EvtFechamentoeFinanceira"),
            "v1_2_2",
            (Action<object>? seed) =>
            {
                var builder = new EvtFechamentoeFinanceiraBuilder("v1_2_2");
    ...
    public static EFinanceiraMessageFactory AddLotes(this EFinanceiraMessageFactory factory)
    {
        ...
        // Registro do RetornoLoteEventos v1.2.0
        factory.RegisterFactory(
            MessageKind.Lote("RetornoLoteEventos"),
            "v1_2_0",
            (Action<object>? seed) =>
            {
                var builder = new RetornoLoteEventosBuilder("v1_2_0");

                // Aplicar configurações do seed se fornecido
                if (seed is Action<RetornoLoteEventosBuilder> configure)
                {
                    configure(builder);
                }
